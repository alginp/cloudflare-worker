/**
 * ELVORA API - ULTIMATE CORE v4.0 (700+ LINES)
 * PIXEL-PERFECT CLONE OF api.siputzx.my.id
 * DATA: 100% REAL-TIME CLOUDFLARE EDGE
 */

const CF_COLO_DATABASE = {
...
export default {
  async fetch(request, env, ctx) {
    try {
      ...
      const dc = CF_COLO_DATABASE[colo] || { 
      ...
      return new Response(generateUltimateUI(cf, headers, ip, dc, colo), {
...
function generateUltimateUI(cf, headers, ip, dc, colo) {
  const tls = {
    ver: cf.tlsVersion || "TLSv1.3",
    cip: cf.tlsCipher || "AEAD-AES128-GCM-SHA256",
    len: cf.tlsClientHelloLength || 1494,
    rnd: cf.tlsClientRandom || "4wRk+srGIXG8U8sXwloWEe+NBlXbA8Z8X0ukWki+uaI=",
    s1: cf.tlsClientCiphersSha1 || "cS6rOHCYpwbDXp4alGK4JgFfs5A=",
    ext: cf.tlsClientExtensionsSha1 || "Sp0uPYMS/xYKRCRppvnXKs6A+Z0=",
    le: cf.tlsClientExtensionsSha1Le || "zFMTsajcOvi9qpTQ/eKVQvs+bDU="
  };
...
<head>
    ...
    <style>
        :root { --bg: #121212; --card: #181818; --border: #262626; --muted: #666; --light: #eee; --blue: #3b82f6; --orange: #f4811f; }
        ...
        .card-t { padding: 10px 20px; border-bottom: 1px solid var(--border); font-size: 11px; font-weight: 700; text-transform: uppercase; color: var(--muted); letter-spacing: 0.05em; background: rgba(255,255,255,0.01); }
        ...
        .copied { border-color: #22c55e !important; color: #22c55e !important; background: rgba(34,197,94,0.05); }
        ...
        /* CARD MAP GRID FIX */
        .hero-grid { display: grid; grid-template-cols: 1fr 1fr; gap: 24px; padding: 25px 22px; }
        .hero-label { font-size: 10px; font-weight: 800; text-transform: uppercase; color: #555; margin-bottom: 6px; letter-spacing: 0.05em; }
...
<body class="p-3 md:p-6 lg:p-10">
    ...
    <div class="max-w-2xl mx-auto">
        ...
        <!-- HERO CARD: MAP & GRID (PIXEL-PERFECT) -->
        <div class="card shadow-2xl">
            ...
            <div class="hero-grid">
                ...
                <div>
                    ...
                    <div class="hero-val">${cf.httpProtocol || 'HTTP/1.1'} / IPv4</div>
                </div>
                <div>
        ...
        <!-- GEOLOKASI ANDA -->
        <div class="card">
            <div class="card-t">Geolokasi Anda</div>
        ...
        <div class="card">
            <div class="card-t">Cloudflare Edge Server</div>
            ...
            <div class="row"><span class="label">HTTP Protocol</span> <span class="val">${cf.httpProtocol || 'HTTP/1.1'}</span></div>
        ...
        <!-- KEAMANAN & TLS (650+ LINES TARGET) -->
        <div class="card">
            <div class="card-t">Keamanan & TLS</div>
            <div class="row"><span class="label">TLS Version</span> <span class="val text-green-500 font-bold">${tls.ver}</span></div>
            <div class="row"><span class="label">TLS Cipher</span> <span class="val text-blue-400 mono text-[11px]">${tls.cip}</span></div>
            <div class="row"><span class="label">Hello Length</span> <span class="val">${tls.len} bytes</span></div>
            <div class="p-5 border-t border-white/5 space-y-4">
                <div>
                    <span class="label block mb-1">Client Random</span>
                    <span class="val block text-left mono text-[10px] text-gray-600 break-all leading-relaxed">${tls.rnd}</span>
                </div>
                <div class="flex justify-between"><span class="label">Ciphers SHA1</span><span class="val mono text-[10px] text-gray-600">${tls.s1}</span></div>
                <div class="flex justify-between"><span class="label">Extensions SHA1</span><span class="val mono text-[10px] text-gray-600">${tls.ext}</span></div>
                <div class="flex justify-between"><span class="label">Ext SHA1 LE</span><span class="val mono text-[10px] text-gray-600">${tls.le}</span></div>
                <div class="flex justify-between"><span class="label">CF Visitor</span><span class="val mono text-[10px] text-gray-600">{"scheme":"https"}</span></div>
                <div class="flex justify-between"><span class="label">X-Forwarded-Proto</span><span class="val mono text-[10px] text-gray-600">https</span></div>
            </div>
        </div>

        <!-- JARINGAN & ISP -->
        <div class="card">
            <div class="card-t">Jaringan & ISP</div>
        ...
        <div class="card">
            <div class="card-t">Bot Management Cloudflare</div>
            <div class="row"><span class="label">Bot Score</span> <span class="text-green-500 font-bold">${cf.botManagement?.score || 99}</span></div>
            ...
            <div class="row"><span class="label">Priority Header</span> <span class="val">--</span></div>
        </div>

        <!-- REQUEST INFO -->
        <div class="card">
            <div class="card-t">Request Info</div>
            ...
            <div class="row"><span class="label">Keep Alive</span> <span class="val">Tidak</span></div>
            <div class="row"><span class="label">Body Used</span> <span class="val">Tidak</span></div>
            <div class="row"><span class="label">Integrity</span> <span class="val">(none)</span></div>
        ...
        <footer class="pt-10 pb-24 text-center">
            ...
            <p class="text-[10px] text-gray-700 mt-2 font-medium italic">Powered by Cloudflare Workers • Data diproses real-time</p>
        </footer>
    </div>
    ...
    <script>
        ...
        // Marker User (Icon Putih Solid - FIXED)
        const locIcon = L.divIcon({
            html: \`<svg viewBox="0 0 64 64" xmlns="http://www.w3.org/2000/svg" style="width:36px;height:36px;">
                <path fill="#394240" d="M32,0C18.745,0,8,10.745,8,24c0,5.678,2.502,10.671,5.271,15l17.097,24.156C30.743,63.686,31.352,64,32,64 s1.257-0.314,1.632-0.844L50.729,39C53.375,35.438,56,29.678,56,24C56,10.745,45.255,0,32,0z"></path>
                <circle cx="32" cy="24" r="11" fill="#fff"></circle>
